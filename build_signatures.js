const fs = require('fs');

// Hosted HTTPS Assets on GitHub (Guarantees 100% compatibility with Outlook, OWA, Gmail, Apple Mail)
const ASSET_BASE = "https://raw.githubusercontent.com/ahmetdogankuday/academic-email-signature/main/assets";

function getAssetSet(base = ASSET_BASE) {
  return {
    logos: {
      bvuYuvarlak: `${base}/bvu-yuvarlak-opt.png`,
      bvuYatay: `${base}/bvu-yatay-opt.png`,
      cochraneYatay: `${base}/cochrane-yatay-opt.png`,
      cochraneStacked: `${base}/cochrane-stacked-opt.png`,
      cochraneCircle: `${base}/cochrane-circle-opt.png`,
    },
    iconsSquare: {
      scholar: `${base}/icons/scholar.png`,
      scopus: `${base}/icons/scopus.png`,
      orcid: `${base}/icons/orcid.png`,
      publons: `${base}/icons/publons.png`,
      researchgate: `${base}/icons/researchgate.png`,
      linkedin: `${base}/icons/linkedin.png`,
    },
    iconsCircle: {
      scholar: `${base}/icons_circle/scholar.png`,
      scopus: `${base}/icons_circle/scopus.png`,
      orcid: `${base}/icons_circle/orcid.png`,
      publons: `${base}/icons_circle/publons.png`,
      researchgate: `${base}/icons_circle/researchgate.png`,
      linkedin: `${base}/icons_circle/linkedin.png`,
    },
    contactBadges: {
      mail: `${base}/icons/mail_badge.png`,
      phone: `${base}/icons/phone_badge.png`,
    }
  };
}

const data = getAssetSet(ASSET_BASE);
const contactBadges = data.contactBadges;

// Turkish Profile Data
const USER_TR = {
  name: "Öğr. Gör. Ahmet Doğan KUDAY",
  cochraneRole: "Çekirdek Üye | Cochrane Türkiye",
  faculty: "Sağlık Hizmetleri Meslek Yüksekokulu",
  program: "İlk ve Acil Yardım Programı",
  emailEdu: "ahmet.kuday@bezmialem.edu.tr",
  phone: "+90 530 114 01 12",
  phoneRaw: "+905301140112",
  web: "https://bezmialem.edu.tr",
  webCochrane: "https://turkey.cochrane.org",
  webShmyo: "https://bezmialem.edu.tr/shmyo",
  webProgram: "https://bezmialem.edu.tr/shmyo/tr/Sayfalar/ilk-ve-acil-yardim/program-tanitimi.aspx",
  webAvesis: "https://avesis.bezmialem.edu.tr/ahmet.kuday",
  links: {
    scholar: "https://scholar.google.com.tr/citations?user=S9J_POsAAAAJ",
    scopus: "https://www.scopus.com/authid/detail.uri?authorId=58751428000",
    orcid: "https://orcid.org/0000-0001-8361-5526",
    publons: "https://publons.com/researcher/GXF-4784-2022",
    researchgate: "https://www.researchgate.net/profile/Ahmet_Kuday",
    linkedin: "https://www.linkedin.com/in/ahmetdogankuday"
  }
};

// English Profile Data (as requested by user)
const USER_EN = {
  name: "Lec. Ahmet Doğan Kuday, DM, MSc, PhD(c)",
  cochraneRole: "Core Member | Cochrane Türkiye",
  faculty: "Vocational School of Health Services",
  program: "Department of First Aid and Emergency",
  emailEdu: "ahmet.kuday@bezmialem.edu.tr",
  phone: "+90 530 114 01 12",
  phoneRaw: "+905301140112",
  web: "https://bezmialem.edu.tr",
  webCochrane: "https://turkey.cochrane.org",
  webShmyo: "https://bezmialem.edu.tr/shmyo",
  webProgram: "https://bezmialem.edu.tr/shmyo/tr/Sayfalar/ilk-ve-acil-yardim/program-tanitimi.aspx",
  webAvesis: "https://avesis.bezmialem.edu.tr/ahmet.kuday",
  links: USER_TR.links
};



// Helper for icon row - Mobile optimized 18px size and 5px spacing
function renderIconRow(iconSet, iconSize = 18, spacing = 5) {
  const platforms = [
    { id: 'scholar', title: 'Google Scholar', url: USER_TR.links.scholar },
    { id: 'scopus', title: 'Scopus', url: USER_TR.links.scopus },
    { id: 'orcid', title: 'ORCID', url: USER_TR.links.orcid },
    { id: 'publons', title: 'Web of Science / Publons', url: USER_TR.links.publons },
    { id: 'researchgate', title: 'ResearchGate', url: USER_TR.links.researchgate },
    { id: 'linkedin', title: 'LinkedIn', url: USER_TR.links.linkedin }
  ];

  return platforms.map(p => `
    <td style="padding-right: ${spacing}px; vertical-align: middle;">
      <a href="${p.url}" target="_blank" title="${p.title}" style="display: inline-block; text-decoration: none; border: 0; line-height: 1;">
        <img src="${iconSet[p.id]}" alt="${p.title}" width="${iconSize}" height="${iconSize}" style="display: block; border: 0; outline: none; width: ${iconSize}px; height: ${iconSize}px; border-radius: 4px;" />
      </a>
    </td>
  `).join('');
}

// --------------------------------------------------------------------------
// DESIGN 1: Modern Akademik Prestij (Sol Dairesel Logo)
// --------------------------------------------------------------------------
function getDesign1(iconType = 'square', lang = 'tr', assetBase = ASSET_BASE) {
  const assets = getAssetSet(assetBase);
  const iconSet = iconType === 'circle' ? assets.iconsCircle : assets.iconsSquare;
  const badges = assets.contactBadges;
  const isEn = lang === 'en';
  const u = isEn ? USER_EN : USER_TR;

  const middleLines = isEn ? `
        <tr>
          <td style="padding-bottom: 2px;">
            <a href="${u.webCochrane}" target="_blank" style="font-size: 11.5px; font-weight: 700; color: #990033; text-decoration: none; display: inline-block; white-space: nowrap;">${u.cochraneRole}</a>
          </td>
        </tr>
        <tr>
          <td style="padding-bottom: 2px;">
            <a href="${u.webShmyo}" target="_blank" style="font-size: 11.5px; font-weight: 600; color: #002b49; text-decoration: none; display: inline-block; white-space: nowrap;">${u.faculty}</a>
          </td>
        </tr>
        <tr>
          <td style="padding-bottom: 7px;">
            <a href="${u.webProgram}" target="_blank" style="font-size: 11px; color: #4a5568; font-weight: 500; text-decoration: none; display: inline-block; white-space: nowrap;">${u.program}</a>
          </td>
        </tr>` : `
        <tr>
          <td style="padding-bottom: 2px;">
            <a href="${u.webShmyo}" target="_blank" style="font-size: 12px; font-weight: 600; color: #990033; text-decoration: none; display: inline-block; white-space: nowrap;">${u.faculty}</a>
          </td>
        </tr>
        <tr>
          <td style="padding-bottom: 7px;">
            <a href="${u.webProgram}" target="_blank" style="font-size: 11.5px; color: #4a5568; font-weight: 500; text-decoration: none; display: inline-block; white-space: nowrap;">${u.program}</a>
          </td>
        </tr>`;

  return `<!-- DESIGN 1 (${lang.toUpperCase()}): MODERN AKADEMIK PRESTIJ -->
<table cellpadding="0" cellspacing="0" border="0" style="font-family: -apple-system, BlinkMacSystemFont, 'Segoe UI', Roboto, Helvetica, Arial, sans-serif; font-size: 12.5px; line-height: 1.4; color: #2d3748; border-collapse: separate; border-spacing: 0; background-color: #ffffff; border: 1px solid #e2e8f0; border-radius: 12px; padding: 12px 16px; max-width: 460px; width: 100%; box-shadow: 0 1px 4px rgba(0,0,0,0.05); mso-table-lspace: 0pt; mso-table-rspace: 0pt;">
  <tr>
    <!-- Sol Yuvarlak Logo -->
    <td style="vertical-align: middle; padding-right: 14px; text-align: center; width: 76px;">
      <div style="background-color: #ffffff; border-radius: 50%; padding: 3px; display: inline-block;">
        <a href="${u.web}" target="_blank" style="text-decoration: none; display: block;">
          <img src="${assets.logos.bvuYuvarlak}" alt="Bezmialem Vakıf Üniversitesi" width="70" height="70" style="display: block; width: 70px; height: 70px; border: 0; outline: none; border-radius: 50%;" />
        </a>
      </div>
    </td>

    <!-- Dikey Bordo Ayraç Çizgisi -->
    <td style="border-left: 2px solid #990033; padding-left: 14px; vertical-align: middle;">
      <table cellpadding="0" cellspacing="0" border="0" style="border-collapse: collapse;">
        <!-- İsim -->
        <tr>
          <td style="padding-bottom: 2px;">
            <a href="${u.webAvesis}" target="_blank" style="font-size: ${isEn ? '14px' : '15px'}; font-weight: 700; color: #002b49; letter-spacing: -0.2px; text-decoration: none; display: inline-block; white-space: nowrap;">${u.name}</a>
          </td>
        </tr>
        ${middleLines}

        <!-- İletişim Bilgileri (GSM Mailin Altında) -->
        <tr>
          <td style="padding-bottom: 8px;">
            <table cellpadding="0" cellspacing="0" border="0" style="border-collapse: collapse;">
              <tr>
                <td style="padding-bottom: 3px;">
                  <table cellpadding="0" cellspacing="0" border="0" style="border-collapse: collapse;">
                    <tr>
                      <td style="vertical-align: middle; padding-right: 6px;">
                        <img src="${badges.mail}" alt="E-posta" width="15" height="15" style="display: block; width: 15px; height: 15px; border: 0; border-radius: 3px;" />
                      </td>
                      <td style="vertical-align: middle; font-size: 11.5px;">
                        <a href="mailto:${u.emailEdu}" style="color: #002b49; text-decoration: none; font-weight: 600; white-space: nowrap;">${u.emailEdu}</a>
                      </td>
                    </tr>
                  </table>
                </td>
              </tr>
              <tr>
                <td>
                  <table cellpadding="0" cellspacing="0" border="0" style="border-collapse: collapse;">
                    <tr>
                      <td style="vertical-align: middle; padding-right: 6px;">
                        <img src="${badges.phone}" alt="Telefon" width="15" height="15" style="display: block; width: 15px; height: 15px; border: 0; border-radius: 3px;" />
                      </td>
                      <td style="vertical-align: middle; font-size: 11.5px;">
                        <a href="tel:${u.phoneRaw}" style="color: #4a5568; text-decoration: none; font-weight: 500; white-space: nowrap;">${u.phone}</a>
                      </td>
                    </tr>
                  </table>
                </td>
              </tr>
            </table>
          </td>
        </tr>

        <!-- Akademik Rozetler -->
        <tr>
          <td>
            <table cellpadding="0" cellspacing="0" border="0" style="border-collapse: collapse;">
              <tr>
                ${renderIconRow(iconSet, 18, 5)}
              </tr>
            </table>
          </td>
        </tr>
      </table>
    </td>
  </tr>
</table>`;
}

// --------------------------------------------------------------------------
// DESIGN 2: Kurumsal Yatay Prestij (Sol Yatay Logo)
// --------------------------------------------------------------------------
function getDesign2(iconType = 'square', lang = 'tr', assetBase = ASSET_BASE) {
  const assets = getAssetSet(assetBase);
  const iconSet = iconType === 'circle' ? assets.iconsCircle : assets.iconsSquare;
  const badges = assets.contactBadges;
  const isEn = lang === 'en';
  const u = isEn ? USER_EN : USER_TR;

  const middleLines = isEn ? `
        <tr>
          <td style="padding-bottom: 2px;">
            <a href="${u.webCochrane}" target="_blank" style="font-size: 11.5px; font-weight: 700; color: #990033; text-decoration: none; display: inline-block; white-space: nowrap;">${u.cochraneRole}</a>
          </td>
        </tr>
        <tr>
          <td style="padding-bottom: 2px;">
            <a href="${u.webShmyo}" target="_blank" style="font-size: 11.5px; font-weight: 600; color: #002b49; text-decoration: none; display: inline-block; white-space: nowrap;">${u.faculty}</a>
          </td>
        </tr>
        <tr>
          <td style="padding-bottom: 7px;">
            <a href="${u.webProgram}" target="_blank" style="font-size: 11px; color: #4a5568; font-weight: 500; text-decoration: none; display: inline-block; white-space: nowrap;">${u.program}</a>
          </td>
        </tr>` : `
        <tr>
          <td style="padding-bottom: 2px;">
            <a href="${u.webShmyo}" target="_blank" style="font-size: 12px; font-weight: 600; color: #990033; text-decoration: none; display: inline-block; white-space: nowrap;">${u.faculty}</a>
          </td>
        </tr>
        <tr>
          <td style="padding-bottom: 7px;">
            <a href="${u.webProgram}" target="_blank" style="font-size: 11.5px; color: #4a5568; font-weight: 500; text-decoration: none; display: inline-block; white-space: nowrap;">${u.program}</a>
          </td>
        </tr>`;

  return `<!-- DESIGN 2 (${lang.toUpperCase()}): KURUMSAL YATAY PRESTIJ -->
<table cellpadding="0" cellspacing="0" border="0" style="font-family: -apple-system, BlinkMacSystemFont, 'Segoe UI', Roboto, Helvetica, Arial, sans-serif; font-size: 12.5px; line-height: 1.4; color: #2d3748; border-collapse: separate; border-spacing: 0; background-color: #ffffff; border: 1px solid #e2e8f0; border-radius: 12px; padding: 12px 16px; max-width: 460px; width: 100%; box-shadow: 0 1px 4px rgba(0,0,0,0.05); mso-table-lspace: 0pt; mso-table-rspace: 0pt;">
  <tr>
    <!-- Sol Yatay Logo -->
    <td style="vertical-align: middle; padding-right: 14px; text-align: center; width: 118px;">
      <div style="background-color: #ffffff; border-radius: 6px; padding: 3px 5px; display: inline-block;">
        <a href="${u.web}" target="_blank" style="text-decoration: none; display: block;">
          <img src="${assets.logos.bvuYatay}" alt="Bezmialem Vakıf Üniversitesi" width="110" style="display: block; width: 110px; height: auto; border: 0; outline: none;" />
        </a>
      </div>
    </td>

    <!-- Dikey Bordo Ayraç Çizgisi -->
    <td style="border-left: 2px solid #990033; padding-left: 14px; vertical-align: middle;">
      <table cellpadding="0" cellspacing="0" border="0" style="border-collapse: collapse;">
        <!-- İsim -->
        <tr>
          <td style="padding-bottom: 2px;">
            <a href="${u.webAvesis}" target="_blank" style="font-size: ${isEn ? '14px' : '15px'}; font-weight: 700; color: #002b49; letter-spacing: -0.2px; text-decoration: none; display: inline-block; white-space: nowrap;">${u.name}</a>
          </td>
        </tr>
        ${middleLines}

        <!-- İletişim Bilgileri (GSM Mailin Altında) -->
        <tr>
          <td style="padding-bottom: 8px;">
            <table cellpadding="0" cellspacing="0" border="0" style="border-collapse: collapse;">
              <tr>
                <td style="padding-bottom: 3px;">
                  <table cellpadding="0" cellspacing="0" border="0" style="border-collapse: collapse;">
                    <tr>
                      <td style="vertical-align: middle; padding-right: 6px;">
                        <img src="${badges.mail}" alt="E-posta" width="15" height="15" style="display: block; width: 15px; height: 15px; border: 0; border-radius: 3px;" />
                      </td>
                      <td style="vertical-align: middle; font-size: 11.5px;">
                        <a href="mailto:${u.emailEdu}" style="color: #002b49; text-decoration: none; font-weight: 600; white-space: nowrap;">${u.emailEdu}</a>
                      </td>
                    </tr>
                  </table>
                </td>
              </tr>
              <tr>
                <td>
                  <table cellpadding="0" cellspacing="0" border="0" style="border-collapse: collapse;">
                    <tr>
                      <td style="vertical-align: middle; padding-right: 6px;">
                        <img src="${badges.phone}" alt="Telefon" width="15" height="15" style="display: block; width: 15px; height: 15px; border: 0; border-radius: 3px;" />
                      </td>
                      <td style="vertical-align: middle; font-size: 11.5px;">
                        <a href="tel:${u.phoneRaw}" style="color: #4a5568; text-decoration: none; font-weight: 500; white-space: nowrap;">${u.phone}</a>
                      </td>
                    </tr>
                  </table>
                </td>
              </tr>
            </table>
          </td>
        </tr>

        <!-- Akademik Rozetler -->
        <tr>
          <td>
            <table cellpadding="0" cellspacing="0" border="0" style="border-collapse: collapse;">
              <tr>
                ${renderIconRow(iconSet, 18, 5)}
              </tr>
            </table>
          </td>
        </tr>
      </table>
    </td>
  </tr>
</table>`;
}

// --------------------------------------------------------------------------
// DESIGN 3: Dikey Çift Kurumsal Logo (Sol Sütun BVU & Cochrane)
// --------------------------------------------------------------------------
function getDesign3(iconType = 'square', lang = 'tr', assetBase = ASSET_BASE) {
  const assets = getAssetSet(assetBase);
  const iconSet = iconType === 'circle' ? assets.iconsCircle : assets.iconsSquare;
  const badges = assets.contactBadges;
  const isEn = lang === 'en';
  const u = isEn ? USER_EN : USER_TR;

  const middleLines = isEn ? `
        <tr>
          <td style="padding-bottom: 2px;">
            <a href="${u.webCochrane}" target="_blank" style="font-size: 11.5px; font-weight: 700; color: #990033; text-decoration: none; display: inline-block; white-space: nowrap;">${u.cochraneRole}</a>
          </td>
        </tr>
        <tr>
          <td style="padding-bottom: 2px;">
            <a href="${u.webShmyo}" target="_blank" style="font-size: 11.5px; font-weight: 600; color: #002b49; text-decoration: none; display: inline-block; white-space: nowrap;">${u.faculty}</a>
          </td>
        </tr>
        <tr>
          <td style="padding-bottom: 7px;">
            <a href="${u.webProgram}" target="_blank" style="font-size: 11px; color: #4a5568; font-weight: 500; text-decoration: none; display: inline-block; white-space: nowrap;">${u.program}</a>
          </td>
        </tr>` : `
        <tr>
          <td style="padding-bottom: 2px;">
            <a href="${u.webShmyo}" target="_blank" style="font-size: 12px; font-weight: 600; color: #990033; text-decoration: none; display: inline-block; white-space: nowrap;">${u.faculty}</a>
          </td>
        </tr>
        <tr>
          <td style="padding-bottom: 7px;">
            <a href="${u.webProgram}" target="_blank" style="font-size: 11.5px; color: #4a5568; font-weight: 500; text-decoration: none; display: inline-block; white-space: nowrap;">${u.program}</a>
          </td>
        </tr>`;

  return `<!-- DESIGN 3 (${lang.toUpperCase()}): DİKEY ÇİFT KURUMSAL LOGO (BVU & COCHRANE) -->
<table cellpadding="0" cellspacing="0" border="0" style="font-family: -apple-system, BlinkMacSystemFont, 'Segoe UI', Roboto, Helvetica, Arial, sans-serif; font-size: 12.5px; line-height: 1.4; color: #2d3748; border-collapse: separate; border-spacing: 0; background-color: #ffffff; border: 1px solid #e2e8f0; border-radius: 12px; padding: 12px 16px; max-width: 460px; width: 100%; box-shadow: 0 1px 4px rgba(0,0,0,0.05); mso-table-lspace: 0pt; mso-table-rspace: 0pt;">
  <tr>
    <!-- Sol Sütunda İki Kurumsal Logo Üst Üste -->
    <td style="vertical-align: middle; padding-right: 14px; text-align: center; width: 108px;">
      <table cellpadding="0" cellspacing="0" border="0" style="border-collapse: collapse; margin: 0 auto; width: 100%;">
        <tr>
          <td align="center" style="padding-bottom: 7px;">
            <a href="${u.web}" target="_blank" style="text-decoration: none; display: block;">
              <img src="${assets.logos.bvuYatay}" alt="Bezmialem Vakıf Üniversitesi" width="102" style="display: block; width: 102px; height: auto; border: 0; margin: 0 auto;" />
            </a>
          </td>
        </tr>
        <tr>
          <td align="center" style="padding-bottom: 7px;">
            <div style="height: 1px; background-color: #edf2f7; width: 80px; margin: 0 auto;"></div>
          </td>
        </tr>
        <tr>
          <td align="center">
            <a href="${u.webCochrane}" target="_blank" style="text-decoration: none; display: block;">
              <img src="${assets.logos.cochraneYatay}" alt="Cochrane Türkiye" width="98" style="display: block; width: 98px; height: auto; border: 0; margin: 0 auto;" />
            </a>
          </td>
        </tr>
      </table>
    </td>

    <!-- Dikey Bordo Ayraç Çizgisi -->
    <td style="border-left: 2px solid #990033; padding-left: 14px; vertical-align: middle;">
      <table cellpadding="0" cellspacing="0" border="0" style="border-collapse: collapse;">
        <!-- İsim -->
        <tr>
          <td style="padding-bottom: 2px;">
            <a href="${u.webAvesis}" target="_blank" style="font-size: ${isEn ? '14px' : '15px'}; font-weight: 700; color: #002b49; letter-spacing: -0.2px; text-decoration: none; display: inline-block; white-space: nowrap;">${u.name}</a>
          </td>
        </tr>
        ${middleLines}

        <!-- İletişim Bilgileri (GSM Mailin Altında) -->
        <tr>
          <td style="padding-bottom: 8px;">
            <table cellpadding="0" cellspacing="0" border="0" style="border-collapse: collapse;">
              <tr>
                <td style="padding-bottom: 3px;">
                  <table cellpadding="0" cellspacing="0" border="0" style="border-collapse: collapse;">
                    <tr>
                      <td style="vertical-align: middle; padding-right: 6px;">
                        <img src="${badges.mail}" alt="E-posta" width="15" height="15" style="display: block; width: 15px; height: 15px; border: 0; border-radius: 3px;" />
                      </td>
                      <td style="vertical-align: middle; font-size: 11.5px;">
                        <a href="mailto:${u.emailEdu}" style="color: #002b49; text-decoration: none; font-weight: 600; white-space: nowrap;">${u.emailEdu}</a>
                      </td>
                    </tr>
                  </table>
                </td>
              </tr>
              <tr>
                <td>
                  <table cellpadding="0" cellspacing="0" border="0" style="border-collapse: collapse;">
                    <tr>
                      <td style="vertical-align: middle; padding-right: 6px;">
                        <img src="${badges.phone}" alt="Telefon" width="15" height="15" style="display: block; width: 15px; height: 15px; border: 0; border-radius: 3px;" />
                      </td>
                      <td style="vertical-align: middle; font-size: 11.5px;">
                        <a href="tel:${u.phoneRaw}" style="color: #4a5568; text-decoration: none; font-weight: 500; white-space: nowrap;">${u.phone}</a>
                      </td>
                    </tr>
                  </table>
                </td>
              </tr>
            </table>
          </td>
        </tr>

        <!-- Akademik Rozetler -->
        <tr>
          <td>
            <table cellpadding="0" cellspacing="0" border="0" style="border-collapse: collapse;">
              <tr>
                ${renderIconRow(iconSet, 18, 5)}
              </tr>
            </table>
          </td>
        </tr>
      </table>
    </td>
  </tr>
</table>`;
}

// Write individual HTML files
// Turkish versions
fs.writeFileSync('imza-1-prestij-dikey.html', getDesign1('square', 'tr'));
fs.writeFileSync('imza-1-tr.html', getDesign1('square', 'tr'));
fs.writeFileSync('imza-2-kurumsal-yatay-prestij.html', getDesign2('square', 'tr'));
fs.writeFileSync('imza-2-tr.html', getDesign2('square', 'tr'));
fs.writeFileSync('imza-3-cift-kurumsal-logo.html', getDesign3('square', 'tr'));
fs.writeFileSync('imza-3-tr.html', getDesign3('square', 'tr'));

// English versions
fs.writeFileSync('imza-1-en.html', getDesign1('square', 'en'));
fs.writeFileSync('imza-2-en.html', getDesign2('square', 'en'));
fs.writeFileSync('imza-3-en.html', getDesign3('square', 'en'));

console.log('Individual signature HTML files (TR and ENG) generated successfully!');

// Generate showcase portal HTML
const showcaseHtml = `<!DOCTYPE html>
<html lang="tr">
<head>
  <meta charset="UTF-8">
  <meta name="viewport" content="width=device-width, initial-scale=1.0">
  <title>Akademik E-Posta İmza Galerisi | Öğr. Gör. Ahmet Doğan KUDAY</title>
  <link rel="preconnect" href="https://fonts.googleapis.com">
  <link rel="preconnect" href="https://fonts.gstatic.com" crossorigin>
  <link href="https://fonts.googleapis.com/css2?family=Plus+Jakarta+Sans:wght@400;500;600;700;800&family=JetBrains+Mono:wght@400;500&display=swap" rel="stylesheet">
  <style>
    :root {
      --primary: #002b49;
      --primary-dark: #001a2c;
      --accent-bordeaux: #990033;
      --bg: #0f172a;
      --surface: #ffffff;
      --text: #0f172a;
      --text-muted: #64748b;
      --border: #e2e8f0;
      --success: #10b981;
    }

    * {
      box-sizing: border-box;
      margin: 0;
      padding: 0;
    }

    body {
      font-family: 'Plus Jakarta Sans', -apple-system, BlinkMacSystemFont, sans-serif;
      background: #f8fafc;
      color: #1e293b;
      min-height: 100vh;
      line-height: 1.5;
    }

    /* Header */
    .header {
      background: linear-gradient(135deg, #001a2c 0%, #002b49 60%, #4a0d1f 100%);
      color: #ffffff;
      padding: 45px 20px 65px;
      text-align: center;
      position: relative;
    }

    .header::after {
      content: '';
      position: absolute;
      bottom: 0;
      left: 0;
      right: 0;
      height: 20px;
      background: #f8fafc;
      border-radius: 20px 20px 0 0;
    }

    .badge-uni {
      display: inline-flex;
      align-items: center;
      gap: 8px;
      background: rgba(255, 255, 255, 0.12);
      backdrop-filter: blur(10px);
      border: 1px solid rgba(255, 255, 255, 0.2);
      padding: 6px 14px;
      border-radius: 99px;
      font-size: 13px;
      font-weight: 600;
      letter-spacing: 0.3px;
      margin-bottom: 14px;
    }

    .header h1 {
      font-size: 30px;
      font-weight: 800;
      letter-spacing: -0.5px;
      margin-bottom: 10px;
    }

    .header p {
      font-size: 15px;
      color: #cbd5e1;
      max-width: 760px;
      margin: 0 auto;
    }

    /* Container */
    .container {
      max-width: 1140px;
      margin: -24px auto 60px;
      padding: 0 20px;
      position: relative;
      z-index: 10;
    }

    /* Quick Guide Card */
    .guide-card {
      background: #ffffff;
      border-radius: 16px;
      padding: 20px 26px;
      margin-bottom: 30px;
      box-shadow: 0 4px 20px rgba(0, 0, 0, 0.05);
      border: 1px solid #e2e8f0;
      display: flex;
      align-items: center;
      justify-content: space-between;
      gap: 20px;
      flex-wrap: wrap;
    }

    .guide-steps {
      display: flex;
      align-items: center;
      gap: 24px;
      flex-wrap: wrap;
    }

    .step-item {
      display: flex;
      align-items: center;
      gap: 10px;
      font-size: 13.5px;
      font-weight: 600;
      color: #334155;
    }

    .step-num {
      width: 28px;
      height: 28px;
      border-radius: 50%;
      background: #002b49;
      color: #ffffff;
      display: flex;
      align-items: center;
      justify-content: center;
      font-size: 13px;
      font-weight: 700;
    }

    .mode-toggle-btn {
      background: #1e293b;
      color: #ffffff;
      display: inline-flex;
      align-items: center;
      gap: 8px;
      font-size: 13px;
      font-weight: 700;
      padding: 10px 18px;
      border-radius: 10px;
      border: 1px solid #334155;
      cursor: pointer;
      transition: all 0.2s;
    }

    .mode-toggle-btn:hover {
      background: #0f172a;
      transform: translateY(-1px);
    }

    .btn-gmail {
      background: #ea4335;
      color: #ffffff;
      display: inline-flex;
      align-items: center;
      gap: 8px;
      font-size: 13px;
      font-weight: 700;
      padding: 10px 18px;
      border-radius: 10px;
      text-decoration: none;
      transition: all 0.2s;
      box-shadow: 0 4px 12px rgba(234, 67, 53, 0.25);
    }

    .btn-gmail:hover {
      background: #d93025;
      transform: translateY(-1px);
    }

    /* Design Group Section */
    .design-group {
      background: #ffffff;
      border-radius: 20px;
      margin-bottom: 36px;
      border: 1px solid #e2e8f0;
      box-shadow: 0 10px 25px -5px rgba(0, 0, 0, 0.05);
      overflow: hidden;
    }

    .group-header {
      padding: 18px 24px;
      background: #fafbfc;
      border-bottom: 1px solid #edf2f7;
      display: flex;
      justify-content: space-between;
      align-items: center;
      flex-wrap: wrap;
      gap: 12px;
    }

    .group-title-wrap {
      display: flex;
      align-items: center;
      gap: 12px;
    }

    .card-tag {
      font-size: 11px;
      font-weight: 700;
      text-transform: uppercase;
      padding: 4px 10px;
      border-radius: 6px;
      letter-spacing: 0.5px;
    }

    .tag-blue { background: #e0f2fe; color: #0369a1; }
    .tag-purple { background: #f3e8ff; color: #7e22ce; }
    .tag-cyan { background: #cffafe; color: #0e7490; }

    .group-title {
      font-size: 16px;
      font-weight: 700;
      color: #0f172a;
    }

    .group-desc {
      font-size: 12.5px;
      color: #64748b;
    }

    /* Side-by-Side Dual Columns */
    .dual-grid {
      display: grid;
      grid-template-columns: 1fr 1fr;
      gap: 0;
      divide-x: 1px solid #edf2f7;
    }

    @media (max-width: 960px) {
      .dual-grid {
        grid-template-columns: 1fr;
      }
    }

    .sub-col {
      display: flex;
      flex-direction: column;
      border-right: 1px solid #edf2f7;
    }

    .sub-col:last-child {
      border-right: none;
    }

    @media (max-width: 960px) {
      .sub-col {
        border-right: none;
        border-bottom: 1px solid #edf2f7;
      }
      .sub-col:last-child {
        border-bottom: none;
      }
    }

    .sub-header {
      padding: 12px 20px;
      background: #f8fafc;
      border-bottom: 1px solid #edf2f7;
      display: flex;
      justify-content: space-between;
      align-items: center;
    }

    .lang-badge {
      display: inline-flex;
      align-items: center;
      gap: 6px;
      font-size: 12px;
      font-weight: 700;
      padding: 3px 9px;
      border-radius: 6px;
      background: #e2e8f0;
      color: #1e293b;
    }

    .lang-badge.lang-en {
      background: #fee2e2;
      color: #991b1b;
    }

    .lang-subtext {
      font-size: 11.5px;
      color: #64748b;
      font-weight: 500;
    }

    /* Preview Canvas */
    .preview-canvas {
      padding: 24px 20px;
      background: #f1f5f9;
      display: flex;
      align-items: center;
      justify-content: center;
      overflow-x: auto;
      flex-grow: 1;
      transition: background 0.3s ease;
    }

    .preview-canvas.dark-mode-preview {
      background: #121212 !important;
    }

    /* Actions Bar */
    .col-bottom {
      padding: 14px 20px;
      background: #fafbfc;
      border-top: 1px solid #edf2f7;
      display: flex;
      justify-content: space-between;
      align-items: center;
      gap: 10px;
      flex-wrap: wrap;
    }

    .btn {
      display: inline-flex;
      align-items: center;
      gap: 6px;
      font-family: inherit;
      font-size: 13px;
      font-weight: 600;
      padding: 8px 14px;
      border-radius: 9px;
      cursor: pointer;
      transition: all 0.2s ease;
      border: 1px solid transparent;
      text-decoration: none;
    }

    .btn-primary {
      background: #002b49;
      color: #ffffff;
      box-shadow: 0 3px 10px rgba(0, 43, 73, 0.2);
    }

    .btn-primary:hover {
      background: #001f35;
      transform: translateY(-1px);
      box-shadow: 0 5px 14px rgba(0, 43, 73, 0.25);
    }

    .btn-secondary {
      background: #ffffff;
      color: #334155;
      border-color: #cbd5e1;
    }

    .btn-secondary:hover {
      background: #f1f5f9;
      color: #0f172a;
    }

    /* Toast Notification */
    .toast {
      position: fixed;
      bottom: 30px;
      right: 30px;
      background: #0f172a;
      color: #ffffff;
      padding: 16px 26px;
      border-radius: 14px;
      box-shadow: 0 12px 30px rgba(0, 0, 0, 0.25);
      display: flex;
      align-items: center;
      gap: 12px;
      font-weight: 600;
      font-size: 14px;
      transform: translateY(100px);
      opacity: 0;
      transition: all 0.3s cubic-bezier(0.16, 1, 0.3, 1);
      z-index: 1000;
      border: 1px solid #334155;
    }

    .toast.show {
      transform: translateY(0);
      opacity: 1;
    }

    .toast-icon {
      color: #10b981;
      font-size: 20px;
    }

    /* Detailed Instructions Section */
    .instructions-section {
      background: #ffffff;
      border-radius: 18px;
      padding: 30px;
      margin-top: 36px;
      border: 1px solid #e2e8f0;
      box-shadow: 0 4px 20px rgba(0, 0, 0, 0.04);
    }

    .instructions-section h2 {
      font-size: 19px;
      font-weight: 800;
      color: #002b49;
      margin-bottom: 18px;
      display: flex;
      align-items: center;
      gap: 10px;
    }

    .instruction-steps {
      display: grid;
      grid-template-columns: repeat(auto-fit, minmax(220px, 1fr));
      gap: 16px;
    }

    .instruction-card {
      background: #f8fafc;
      border: 1px solid #e2e8f0;
      border-radius: 12px;
      padding: 18px;
    }

    .instruction-card h3 {
      font-size: 13.5px;
      font-weight: 700;
      color: #002b49;
      margin-bottom: 6px;
      display: flex;
      align-items: center;
      gap: 8px;
    }

    .instruction-card p {
      font-size: 12.5px;
      color: #64748b;
      line-height: 1.45;
    }

    code {
      background: #e2e8f0;
      padding: 2px 6px;
      border-radius: 4px;
      font-family: 'JetBrains Mono', monospace;
      font-size: 12px;
      color: #0f172a;
    }
  </style>
</head>
<body>

  <!-- Header -->
  <header class="header">
    <div class="badge-uni">
      <span>🏛️</span> <a href="https://bezmialem.edu.tr/shmyo" target="_blank" style="color: inherit; text-decoration: none;">Sağlık Hizmetleri Meslek Yüksekokulu</a> • <a href="https://bezmialem.edu.tr/shmyo/tr/Sayfalar/ilk-ve-acil-yardim/program-tanitimi.aspx" target="_blank" style="color: inherit; text-decoration: none;">İlk ve Acil Yardım Programı</a>
    </div>
    <h1>Akademik E-Posta İmza Galerisi</h1>
    <p>🇹🇷 Türkçe &amp; 🇬🇧 English Yan Yana • 📱 Mobil Uyumlu &amp; 🌙 Dark Mode Korumalı Kurumsal Kart Tasarımları</p>
  </header>

  <div class="container">

    <!-- Quick Guide & Dark Mode Switcher -->
    <div class="guide-card">
      <div class="guide-steps">
        <div class="step-item">
          <div class="step-num">1</div>
          <span>Beğendiğiniz tasarımın (TR veya ENG) altındaki <strong>"✨ İmzayı Kopyala"</strong> butonuna basın</span>
        </div>
        <div class="step-item">
          <div class="step-num">2</div>
          <span>Gmail veya Bezmialem Webmail (Outlook / OWA) imza kutusuna <code>Cmd + V</code> ile yapıştırın</span>
        </div>
      </div>
      <div style="display: flex; gap: 10px; align-items: center; flex-wrap: wrap;">
        <button id="toggleDarkBtn" class="mode-toggle-btn" onclick="toggleDarkPreview()">
          🌙 Karanlık Modu Test Et
        </button>
        <a href="https://mail.google.com/mail/u/0/#settings/general" target="_blank" class="btn-gmail">
          <span>⚙️</span> Gmail Ayarları &rarr;
        </a>
        <a href="https://mail.bezmialem.edu.tr/owa/" target="_blank" class="btn-gmail" style="background: linear-gradient(135deg, #0078d4, #005a9e);">
          <span>✉️</span> Bezmialem Webmail (OWA) &rarr;
        </a>
      </div>
    </div>

    <!-- DESIGN GROUP 1 -->
    <div class="design-group">
      <div class="group-header">
        <div class="group-title-wrap">
          <span class="card-tag tag-blue">Tasarım 1</span>
          <div>
            <div class="group-title">Modern Akademik Prestij (Sol Dairesel Logo)</div>
            <div class="group-desc">Dairesel BVU arması, dikey bordo çizgi, üstte mail, altta GSM ve 6 akademik rozet.</div>
          </div>
        </div>
      </div>
      <div class="dual-grid">
        <!-- TR Column -->
        <div class="sub-col">
          <div class="sub-header">
            <span class="lang-badge">🇹🇷 Türkçe</span>
            <span class="lang-subtext">Öğr. Gör. Ahmet Doğan KUDAY</span>
          </div>
          <div class="preview-canvas" id="preview-1-tr">
            ${getDesign1('square', 'tr', './assets')}
          </div>
          <div class="col-bottom">
            <button class="btn btn-secondary" onclick="copyRawHtml('preview-1-tr')">📋 HTML</button>
            <button class="btn btn-primary" onclick="copyRichSignature('preview-1-tr')">✨ İmzayı Kopyala (TR)</button>
          </div>
        </div>
        <!-- ENG Column -->
        <div class="sub-col">
          <div class="sub-header">
            <span class="lang-badge lang-en">🇬🇧 English</span>
            <span class="lang-subtext">Lec. Ahmet Doğan Kuday, DM, MSc, PhD(c)</span>
          </div>
          <div class="preview-canvas" id="preview-1-en">
            ${getDesign1('square', 'en', './assets')}
          </div>
          <div class="col-bottom">
            <button class="btn btn-secondary" onclick="copyRawHtml('preview-1-en')">📋 HTML</button>
            <button class="btn btn-primary" onclick="copyRichSignature('preview-1-en')">✨ Copy Signature (ENG)</button>
          </div>
        </div>
      </div>
    </div>

    <!-- DESIGN GROUP 2 -->
    <div class="design-group">
      <div class="group-header">
        <div class="group-title-wrap">
          <span class="card-tag tag-purple">Tasarım 2</span>
          <div>
            <div class="group-title">Kurumsal Yatay Prestij (Sol Yatay Logo)</div>
            <div class="group-desc">Kurumsal yatay BVU logosu, dikey bordo çizgi, üstte mail, altta GSM ve 6 akademik rozet.</div>
          </div>
        </div>
      </div>
      <div class="dual-grid">
        <!-- TR Column -->
        <div class="sub-col">
          <div class="sub-header">
            <span class="lang-badge">🇹🇷 Türkçe</span>
            <span class="lang-subtext">Öğr. Gör. Ahmet Doğan KUDAY</span>
          </div>
          <div class="preview-canvas" id="preview-2-tr">
            ${getDesign2('square', 'tr', './assets')}
          </div>
          <div class="col-bottom">
            <button class="btn btn-secondary" onclick="copyRawHtml('preview-2-tr')">📋 HTML</button>
            <button class="btn btn-primary" onclick="copyRichSignature('preview-2-tr')">✨ İmzayı Kopyala (TR)</button>
          </div>
        </div>
        <!-- ENG Column -->
        <div class="sub-col">
          <div class="sub-header">
            <span class="lang-badge lang-en">🇬🇧 English</span>
            <span class="lang-subtext">Lec. Ahmet Doğan Kuday, DM, MSc, PhD(c)</span>
          </div>
          <div class="preview-canvas" id="preview-2-en">
            ${getDesign2('square', 'en', './assets')}
          </div>
          <div class="col-bottom">
            <button class="btn btn-secondary" onclick="copyRawHtml('preview-2-en')">📋 HTML</button>
            <button class="btn btn-primary" onclick="copyRichSignature('preview-2-en')">✨ Copy Signature (ENG)</button>
          </div>
        </div>
      </div>
    </div>

    <!-- DESIGN GROUP 3 -->
    <div class="design-group">
      <div class="group-header">
        <div class="group-title-wrap">
          <span class="card-tag tag-cyan">Tasarım 3</span>
          <div>
            <div class="group-title">Dikey Çift Kurumsal Logo (BVU &amp; Cochrane Ortaklık)</div>
            <div class="group-desc">Sol sütunda üst üste konumlandırılmış iki resmi kurumsal logo, dikey bordo çizgi ve tam akademik künye.</div>
          </div>
        </div>
      </div>
      <div class="dual-grid">
        <!-- TR Column -->
        <div class="sub-col">
          <div class="sub-header">
            <span class="lang-badge">🇹🇷 Türkçe</span>
            <span class="lang-subtext">Öğr. Gör. Ahmet Doğan KUDAY</span>
          </div>
          <div class="preview-canvas" id="preview-3-tr">
            ${getDesign3('square', 'tr', './assets')}
          </div>
          <div class="col-bottom">
            <button class="btn btn-secondary" onclick="copyRawHtml('preview-3-tr')">📋 HTML</button>
            <button class="btn btn-primary" onclick="copyRichSignature('preview-3-tr')">✨ İmzayı Kopyala (TR)</button>
          </div>
        </div>
        <!-- ENG Column -->
        <div class="sub-col">
          <div class="sub-header">
            <span class="lang-badge lang-en">🇬🇧 English</span>
            <span class="lang-subtext">Lec. Ahmet Doğan Kuday, DM, MSc, PhD(c)</span>
          </div>
          <div class="preview-canvas" id="preview-3-en">
            ${getDesign3('square', 'en', './assets')}
          </div>
          <div class="col-bottom">
            <button class="btn btn-secondary" onclick="copyRawHtml('preview-3-en')">📋 HTML</button>
            <button class="btn btn-primary" onclick="copyRichSignature('preview-3-en')">✨ Copy Signature (ENG)</button>
          </div>
        </div>
      </div>
    </div>

    <!-- Detailed Instructions Section -->
    <div class="instructions-section">
      <h2>📌 Mobil &amp; Dark Mode Uyum Rehberi</h2>
      <div class="instruction-steps">
        <div class="instruction-card">
          <h3>1. Neden Kartvizit Formatı?</h3>
          <p>Telefonlarda ve karanlık modda arka plan siyah olduğunda koyu lacivert yazılar ve şeffaf logolar görünmez hale gelir. Kartvizit formatı imzanızı her ortamda %100 net ve okunabilir tutar.</p>
        </div>
        <div class="instruction-card">
          <h3>2. Mobil Genişlik (350-460px)</h3>
          <p>Tüm imzalar maksimum 460px aralığında optimize edildi. iPhone SE dahil en dar telefon ekranlarında bile satırlar bölünmez ve sağa-sola taşma yaşanmaz.</p>
        </div>
        <div class="instruction-card">
          <h3>3. Gmail'e Kolay Yapıştırma</h3>
          <p>Mavi butona tıkladığınızda imza tüm stilleriyle kopyalanır. Gmail ayarlarında <code>Cmd + V</code> ile yapıştırmanız yeterlidir.</p>
        </div>
        <div class="instruction-card">
          <h3>4. Otomatik Linkler</h3>
          <p>İsminiz AVESİS profilinize, Cochrane unvanınız resmi web sitesine, yüksekokul ve program yazıları ilgili bölümlere yönlendirir.</p>
        </div>
      </div>
    </div>

  </div>

  <!-- Toast Notification -->
  <div id="toast" class="toast">
    <span class="toast-icon">✓</span>
    <span id="toast-message">İmza panoya başarıyla kopyalandı!</span>
  </div>

  <script>
    let isDark = false;
    function toggleDarkPreview() {
      isDark = !isDark;
      const canvases = document.querySelectorAll('.preview-canvas');
      const btn = document.getElementById('toggleDarkBtn');
      canvases.forEach(c => {
        if (isDark) {
          c.classList.add('dark-mode-preview');
        } else {
          c.classList.remove('dark-mode-preview');
        }
      });
      if (isDark) {
        btn.innerHTML = '☀️ Aydınlık Moda Dön';
        btn.style.background = '#475569';
        showToast('🌙 Karanlık mod simülasyonu aktif! İmzalar kartvizit formatında net görünür.');
      } else {
        btn.innerHTML = '🌙 Karanlık Modu Test Et';
        btn.style.background = '#1e293b';
      }
    }

    function showToast(msg) {
      const toast = document.getElementById('toast');
      document.getElementById('toast-message').textContent = msg;
      toast.classList.add('show');
      setTimeout(() => {
        toast.classList.remove('show');
      }, 3500);
    }

    const REMOTE_ASSET_BASE = 'https://raw.githubusercontent.com/ahmetdogankuday/academic-email-signature/main/assets';

    function makeAssetUrlsAbsolute(html) {
      return html
        .split('src="./assets/').join('src="' + REMOTE_ASSET_BASE + '/')
        .split('src="assets/').join('src="' + REMOTE_ASSET_BASE + '/');
    }

    async function copyRichSignature(elementId) {
      const container = document.getElementById(elementId);
      const htmlContent = makeAssetUrlsAbsolute(container.innerHTML.trim());

      try {
        const blobHtml = new Blob([htmlContent], { type: 'text/html' });
        const blobText = new Blob([container.innerText], { type: 'text/plain' });
        const item = new ClipboardItem({
          'text/html': blobHtml,
          'text/plain': blobText
        });
        await navigator.clipboard.write([item]);
        showToast('✓ İmza kopyalandı! Şimdi Gmail imza ayarlarında Cmd+V ile yapıştırabilirsiniz.');
      } catch (err) {
        const tempDiv = document.createElement('div');
        tempDiv.style.position = 'fixed';
        tempDiv.style.left = '-9999px';
        tempDiv.innerHTML = htmlContent;
        document.body.appendChild(tempDiv);
        const range = document.createRange();
        range.selectNode(tempDiv);
        window.getSelection().removeAllRanges();
        window.getSelection().addRange(range);
        try {
          document.execCommand('copy');
          window.getSelection().removeAllRanges();
          showToast('✓ İmza kopyalandı! Şimdi Gmail imza ayarlarında Cmd+V ile yapıştırabilirsiniz.');
        } catch (fallbackErr) {
          showToast('Kopyalama başarısız oldu, lütfen fareyle seçip Cmd+C yapınız.');
        } finally {
          document.body.removeChild(tempDiv);
        }
      }
    }

    async function copyRawHtml(elementId) {
      const container = document.getElementById(elementId);
      const htmlContent = makeAssetUrlsAbsolute(container.innerHTML.trim());
      try {
        await navigator.clipboard.writeText(htmlContent);
        showToast('✓ HTML kaynak kodu panoya kopyalandı!');
      } catch (e) {
        showToast('HTML kodu kopyalanamadı.');
      }
    }
  </script>
</body>
</html>`;

fs.writeFileSync('index.html', showcaseHtml);
console.log('Build completed successfully!');
